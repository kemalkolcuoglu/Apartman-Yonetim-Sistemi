...
{
	public partial class YakitGoruntule : Form
	{
		string filtre;
		string id, daire;
		DataTable dt;
		...
		{
			id = dataGridView1.CurrentRow.Cells["Yakıt Makbuz No"].Value.ToString();
		...
		private void bt_filtrele_Click(object sender, EventArgs e)
		{
			if (!rb_daireNo.Checked && !rb_tarih.Checked || textBox1.Text == "")
			{
				...
				dataGridView1.DataSource = VeriIslemleri.TabloDoldur("Yakit", VeriIslemleri.Filtre("Yakit", filtre, aranan: textBox1.Text));
		...
		private void bt_sil_Click(object sender, EventArgs e)
		{
			...
			else
			{
				...
				if (cevap.ToString() == "OK")
					durum = VeriIslemleri.VeriSil("Yakit", "YakitID", id);
			}
			if (durum)
			{
		...
		private void bt_toplamTutar_Click(object sender, EventArgs e)
		{
			string[] dizi = VeriIslemleri.YakitToplamBul(daireNo: daire);
			MessageBox.Show(daire + " Nolu Dairenin Toplam Ödediği Aidat Tutarı\n-> "
				+ dizi[0] + "₺\n"
				+ "Alınan Toplam Kontör Miktarı\n-> " + dizi[1]);
		}

		private void YakitGoruntule_Resize(object sender, EventArgs e)
		{
